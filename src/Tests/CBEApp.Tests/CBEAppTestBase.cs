...
namespace CBEApp.Tests
{
    public abstract class CBEAppTestBase : AbpIntegratedTestBase<CBEAppTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CBEAppTestBase()
        {
        ...
        protected void UsingDbContext(Action<CBEAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CBEAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CBEAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CBEAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CBEAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CBEAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CBEAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CBEAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CBEAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CBEAppDbContext, Task<T>> func)
        {
            T result;
